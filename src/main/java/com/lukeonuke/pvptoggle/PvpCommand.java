...
public class PvpCommand implements CommandExecutor {
    ...
    public boolean onCommand(
        ...
        if (args.length == 0) {
            ...
            if (PvpService.isInCombat(player)) {
                player.sendMessage("§cDu kan inte ändra PvP under combat!");
                return true;
            }

            if (!PvpService.isPvpCooldownDone(player) && PvpService.isPvpEnabled(player)) {
                long remainingCooldownMs =
                        PvpService.getPvpCooldownTimestamp(player).toEpochMilli()
                                + (cs.getCooldownDuration() * 1000 + 1000)
                                - Instant.now().toEpochMilli();

                commandSender.sendMessage(
                        ChatFormatterService.addPrefix(cs.getCooldownMessage())
                                .replace("%s", ChatFormatterService.formatTime(remainingCooldownMs))
                );
                return true;
            }
            ...
            commandSender.sendMessage(
                    ChatFormatterService.addPrefix(cs.getToggleMessage())
                            .replace("%s", ChatFormatterService.booleanHumanReadable(isPvpEnabled))
            );
        }
            ...
            for (String arg : args) {
                ...
                if (player == null) {
                    commandSender.sendMessage(
                            ChatFormatterService.addPrefix(
                                    cs.getNotFoundMessage().replace("%s", arg)
                            )
                    );
                    continue;
                }
                ...
                if (PvpService.isInCombat(player)) {
                    commandSender.sendMessage("§c" + player.getName() + " är i combat och kan inte ändra PvP!");
                    continue;
                }
                ...
                commandSender.sendMessage(
                        ChatFormatterService.addPrefix(cs.getRemoteToggleMessage())
                                .replace("%s", player.getName())
                                .replace("%r", ChatFormatterService.booleanHumanReadable(!isPvpEnabled))
                );
            }
        }
